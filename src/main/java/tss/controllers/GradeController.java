...
//    @Authorization
    ...
    public ResponseEntity<GetGradeResponse> getgrade(@RequestBody GetGradeRequest request)
    {
        ...
        if(!ret.isPresent())
        {
            return new ResponseEntity<>(new GetGradeResponse("no such class"),HttpStatus.BAD_REQUEST);
        }
        ClassEntity classEntity = ret.get();
        for(ClassRegistrationEntity classRegistrationEntity:classEntity.getClassRegistrations())
            socres.add(classRegistrationEntity.getScore());
        return new ResponseEntity<>(new GetGradeResponse("ok",socres),HttpStatus.OK);
    }
    @PutMapping(path="/add")
